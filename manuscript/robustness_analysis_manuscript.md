...
Treatment: mean change −19.80 (SD 13.80). Placebo: −8.40 (SD 11.88). Welch t-test: difference −11.40 (95% CI −18.35 to −4.44), t = −3.286, df = 52.3, **p = 0.0018**. The treatment group includes one extreme responder (−52.0, subject 14); the placebo group one minimal responder (+3.0, subject 22).
...
For a significant primary result: score > 70 supports confirmatory-grade reporting; (55, 70] calls for transparent component reporting, clinical review of the worst-case removal set, and a rank-based supplementary analysis; ≤ 55 indicates the profile typical of chance findings — treat as hypothesis-generating. Worst-case fragility below ~5% of the sample should always trigger review of the removed subjects, whatever the score. For non-significant results, use the components descriptively; the score bands do not apply (Section 2.3).
