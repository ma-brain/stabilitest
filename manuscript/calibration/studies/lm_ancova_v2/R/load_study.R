# Load shared calibration helpers plus the isolated ANCOVA v2 study sources.
# Reuses v1 generator/power helpers without mutating v1 study files.

.lm_ancova_v2_study_script_path <- function() {
  source_files <- vapply(
    sys.frames(),
    function(frame) {
...
.lm_ancova_v2_study_script_file <- tryCatch(
  .lm_ancova_v2_study_script_path(),
...
.lm_ancova_v2_study_root <- function(script_path = .lm_ancova_v2_study_script_file) {
  ...
  stop("Unable to locate the lm_ancova_v2 study root", call. = FALSE)
}

.lm_ancova_v2_project_root <- function(study_root = .lm_ancova_v2_study_root()) {
  normalizePath(file.path(study_root, "..", "..", "..", ".."), mustWork = TRUE)
}

.lm_ancova_v2_v1_study_root <- function(project_root) {
  normalizePath(
...
load_lm_ancova_v2_study <- function(project_root = NULL, envir = parent.frame()) {
  study_root <- .lm_ancova_v2_study_root()
  if (is.null(project_root)) {
    project_root <- .lm_ancova_v2_project_root(study_root)
  } else {
  ...
  # Reuse immutable v1 analytic helpers; do not source v1 adapter/thresholds.
  v1_root <- .lm_ancova_v2_v1_study_root(project_root)
  ...
  if (!file.exists(scenario_file)) {
    stop("ANCOVA v2 study scenario configuration is missing", call. = FALSE)
  }
