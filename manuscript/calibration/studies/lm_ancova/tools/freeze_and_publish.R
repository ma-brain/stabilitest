# Atomic ANCOVA publication writer with fail-closed destination checks.

`%||%` <- function(left, right) if (is.null(left)) right else left

.lm_ancova_write_artifact <- function(value, path) {
...
lm_ancova_hash_ledger <- function(paths) {
  ...
  setNames(lapply(paths, function(path) {
    ...
    list(
      path = path,
      hash = unname(as.character(tools::md5sum(path))),
      bytes = unname(file.info(path)$size)
    )
  }), names(paths) %||% unlist(paths))
}

lm_ancova_publish_atomic <- function(artifacts, destination,
  ...
  staging <- tempfile("ancova-publish-", tmpdir = dirname(destination))
  ...
  file_map <- list(
    ...
    power_verification = "power_verification.csv",
    candidate = "candidate.rds",
    validation = "validation.rds",
    registry = "registry.rds",
    training_manifest = "training_manifest.rds",
    validation_manifest = "validation_manifest.rds"
  )

  written <- list()
  for (name in names(file_map)) {
    if (is.null(artifacts[[name]])) next
    written[[file_map[[name]]]] <- .lm_ancova_write_artifact(
      artifacts[[name]], file.path(staging, file_map[[name]])
    )
  }
  if (!is.null(artifacts$registry)) {
    written[["registry.csv"]] <- .lm_ancova_write_artifact(
