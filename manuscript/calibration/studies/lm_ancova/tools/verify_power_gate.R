...
# Primary-test-only production power gate for the isolated ANCOVA study.
# Does not compute robustness scores.
...
.study_root <- function() {
  ...
  normalizePath(
    file.path("manuscript", "calibration", "studies", "lm_ancova"),
    mustWork = TRUE
  )
}
...
canonical <- scenarios[
  scenarios$design_layer %in% c("core", "validation") &
    scenarios$calibration_unit == "lm_ancova",
  ,
  drop = FALSE
]
...
message(sprintf(
  "Wrote %s (%d scenarios, %d failures, sha256=%s)",
  out, nrow(table), meta$n_fail, meta$sha256
))
