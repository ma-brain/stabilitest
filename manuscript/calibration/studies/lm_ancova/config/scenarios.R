# Isolated ANCOVA calibration scenario contract.

.lm_ancova_truth_power <- function(truth_class) {
  switch(
    as.character(truth_class),
    null = 0,
    borderline = 0.60,
    clear = 0.90,
    stop(sprintf("unknown ANCOVA truth class: %s", truth_class), call. = FALSE)
  )
}

.lm_ancova_scenario_row <- function(scenario_id, design_layer, truth_class,
                                    sample_size, scenario_seed, generator,
                                    screening_target = 100L) {
  target_power <- .lm_ancova_truth_power(truth_class)
  tibble::tibble(
...
.lm_ancova_grid_rows <- function(grid, design_layer, seed_start) {
  ...
  for (i in seq_len(nrow(grid))) {
    ...
    target_power <- .lm_ancova_truth_power(truth_class)
    scenario_id <- sprintf(
      "lm_ancova_%s_n%d_r2_%02d_%s",
      design_layer,
      n,
      as.integer(round(100 * baseline_r2)),
      truth_class
    )
    rows[[i]] <- .lm_ancova_scenario_row(
      scenario_id = scenario_id,
      design_layer = design_layer,
      truth_class = truth_class,
      sample_size = n,
...
.lm_ancova_stress_rows <- function(seed_start) {
  specs <- list(
    list(
      id = "lm_ancova_stress_allocation_2to1",
      truth = "borderline",
      n = 120L,
      generator = list(
    ...
    list(
      id = "lm_ancova_stress_heteroscedastic",
      truth = "borderline",
      n = 120L,
      generator = list(
        n = 120L, baseline_r2 = 0.40, target_power = 0.60,
    ...
    list(
      id = "lm_ancova_stress_heavy_tails",
      truth = "borderline",
      n = 120L,
      generator = list(
        n = 120L, baseline_r2 = 0.40, target_power = 0.60,
    ...
    list(
      id = "lm_ancova_stress_missing_baseline",
      truth = "borderline",
      n = 120L,
      generator = list(
        n = 120L, baseline_r2 = 0.40, target_power = 0.60,
    ...
    list(
      id = "lm_ancova_stress_nonlinear_baseline",
      truth = "borderline",
      n = 120L,
      generator = list(
        n = 120L, baseline_r2 = 0.40, target_power = 0.60,
    ...
    list(
      id = "lm_ancova_stress_interaction",
      truth = "borderline",
      n = 120L,
      generator = list(
        n = 120L, baseline_r2 = 0.40, target_power = 0.60,
...
#' Return the isolated ANCOVA calibration scenario contract.
#'
...
lm_ancova_scenarios <- function() {
  ...
  dplyr::bind_rows(
    .lm_ancova_grid_rows(training, "core", seed_start = 31001L),
    .lm_ancova_grid_rows(validation, "validation", seed_start = 32001L),
    .lm_ancova_stress_rows(seed_start = 33001L)
  )
}
