...
# Isolated ANCOVA v3 Track E entry point. Reuses the shared runner while
# keeping scenario lookup and manifests local to this study.

.lm_ancova_v3_runner_script <- function() {
  ...
  study_files <- files[grepl(
    paste0("studies", .Platform$file.sep, "lm_ancova_v3", .Platform$file.sep),
    files,
    fixed = TRUE
  )]
  ...
  normalizePath(
    file.path(
      "manuscript", "calibration", "studies", "lm_ancova_v3", "run_calibration.R"
    ),
    mustWork = TRUE
  )
}

.lm_ancova_v3_runner_study_root <- function(script = .lm_ancova_v3_runner_script()) {
  normalizePath(dirname(script), mustWork = TRUE)
}

.lm_ancova_v3_runner_project_root <- function(
    study_root = .lm_ancova_v3_runner_study_root()) {
  normalizePath(file.path(study_root, "..", "..", "..", ".."), mustWork = TRUE)
}

prepare_lm_ancova_v3_runner <- function(project_root = NULL, study_root = NULL,
                                        envir = parent.frame()) {
  if (is.null(study_root)) study_root <- .lm_ancova_v3_runner_study_root()
  if (is.null(project_root)) {
    project_root <- .lm_ancova_v3_runner_project_root(study_root)
  }
  ...
  envir$load_lm_ancova_v3_study(project_root = project_root, envir = envir)
  ...
  envir$calibration_scenarios <- envir$lm_ancova_v3_scenarios
  envir$.calibration_adapter_for_scenario <- function(scenario, envir = parent.frame()) {
    ...
    if (!identical(family, "lm")) {
      stop(
        sprintf(
          "ANCOVA v3 study runner supports only lm scenarios, got: %s",
          family
        ),
        call. = FALSE
      )
...
run_lm_ancova_v3_calibration <- function(args = commandArgs(trailingOnly = TRUE),
                                         project_root = NULL,
                                         study_root = NULL,
                                         hooks = list()) {
  runner_env <- environment()
  prepared <- prepare_lm_ancova_v3_runner(
    project_root = project_root,
    study_root = study_root,
    envir = runner_env
  )
  script <- .lm_ancova_v3_runner_script()
...
.lm_ancova_v3_runner_is_direct <- function() {
  identical(sys.nframe(), 0L) ||
    length(grep("^--file=.*studies/lm_ancova_v3/run_calibration[.]R$",
                commandArgs(trailingOnly = FALSE))) > 0L
}

if (isTRUE(.lm_ancova_v3_runner_is_direct()) &&
    identical(sys.nframe(), 0L)) {
  run_lm_ancova_v3_calibration()
}
