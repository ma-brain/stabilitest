# Load shared calibration helpers plus the isolated binary-proportion study sources.

.binary_proportion_study_script_path <- function() {
  source_files <- vapply(
    sys.frames(),
    function(frame) {
...
.binary_proportion_study_script_file <- tryCatch(
  .binary_proportion_study_script_path(),
  error = function(error) NULL
)

.binary_proportion_study_root <- function(script_path = .binary_proportion_study_script_file) {
  ...
  repeat {
    ...
    nested <- file.path(
      candidate, "manuscript", "calibration", "studies", "binary_proportion"
    )
  ...
  stop("Unable to locate the binary_proportion study root", call. = FALSE)
}

.binary_proportion_project_root <- function(study_root = .binary_proportion_study_root()) {
  normalizePath(file.path(study_root, "..", "..", "..", ".."), mustWork = TRUE)
}

load_binary_proportion_study <- function(project_root = NULL,
                                          envir = parent.frame()) {
  study_root <- .binary_proportion_study_root()
  if (is.null(project_root)) {
    project_root <- .binary_proportion_project_root(study_root)
  } else {
  ...
  if (!file.exists(scenario_file)) {
    stop("binary_proportion study scenario configuration is missing",
         call. = FALSE)
  }
